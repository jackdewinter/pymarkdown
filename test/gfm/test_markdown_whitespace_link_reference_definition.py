"""
Testing various aspects of whitespaces around link reference definitions.
"""

from test.utils import act_and_assert

import pytest

enable_tables_config_map = {"extensions": {"markdown-tables": {"enabled": True}}}


def test_whitespaces_lrd_with_spaces_before_x() -> None:
    """
    Test case:  LRD preceeded by spaces.
    """

    # Arrange
    source_markdown = """   [fred]: /url
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,4):True:   :fred:: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_too_many_spaces_before() -> None:
    """
    Test case:  LRD preceeded by spaces.

    test_whitespaces_tables_with_too_many_spaces_before
    """

    # Arrange
    source_markdown = """[fred]
    [fred]: /url"""
    expected_tokens = [
        "[para(1,1):\n    ]",
        "[text(1,1):[fred]\n[fred]: /url::\n]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p>[fred]
[fred]: /url</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_too_many_spaces_before_after_first() -> None:
    """
    Test case:  LRD preceeded by spaces.

    test_whitespaces_tables_with_too_many_spaces_before_after_first
    """

    # Arrange
    source_markdown = """[fred]:
    /url
    'title'
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred::\n    :/url::\n    :title:'title':]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:title::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_tabs_before_x() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_whitespaces_tables_with_tabs_before_x
    """

    # Arrange
    source_markdown = """\t[fred]: /url
[fred]"""
    expected_tokens = [
        "[icode-block(1,5):\t:]",
        "[text(1,5):[fred]: /url:]",
        "[end-icode-block:::False]",
        "[para(2,1):]",
        "[text(2,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<pre><code>[fred]: /url
</code></pre>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_repeat() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """\t[abc]: /url
\t[def]: /url
[abc]
[def]"""
    expected_tokens = [
        "[icode-block(1,5):\t:\n\t]",
        "[text(1,5):[abc]: /url\n[def]: /url:]",
        "[end-icode-block:::False]",
        "[para(3,1):\n]",
        "[text(3,1):[abc]\n[def]::\n]",
        "[end-para:::True]",
    ]
    expected_gfm = """<pre><code>[abc]: /url
[def]: /url
</code></pre>
<p>[abc]
[def]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_before() -> None:
    """
    Test case:  LRD preceeded by form feeds.
    """

    # Arrange
    source_markdown = """\u000c[fred]: /url
[fred]"""
    expected_tokens = [
        "[para(1,1):\u000c\n]",
        "[text(1,1):[fred]: /url\n[fred]::\n]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p>[fred]: /url
[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_within_ulist_x() -> None:
    """
    Test case: "LRD" within a single unordered list item.
    """

    # Arrange
    source_markdown = """- [fred]: /url
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2:]",
        "[link-ref-def(1,3):True::fred:: :/url:::::]",
        "[end-ulist:::True]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li></li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_within_ulist_a() -> None:
    """
    Test case: "LRD" within a single unordered list item.
    """

    # Arrange
    source_markdown = """- [fred]: /url 'abc'
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2:]",
        "[link-ref-def(1,3):True::fred:: :/url:: :abc:'abc':]",
        "[end-ulist:::True]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:abc::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li></li>
</ul>
<p><a href="/url" title="abc">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_x() -> None:
    """
    Test case: "LRD" within a single unordered list item, preceeded by a blank line and just enough spaces.
        The blank line is required as LRDs cannot interrupt a paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_within_ulist
    """

    # Arrange
    source_markdown = """- abc

  [fred]: /url
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n  ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,3):True::fred:: :/url:::::]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_a() -> None:
    """
    Test case: "LRD" within a single unordered list item, preceeded by a blank line and just enough spaces.
        The blank line is required as LRDs cannot interrupt a paragraph.
    """

    # Arrange
    source_markdown = """- abc
  [fred]: /url
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::  \n]",
        "[para(1,3):\n\n]",
        "[text(1,3):abc\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-ulist:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]: /url
[fred]</li>
</ul>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_b() -> None:
    """
    Test case: "LRD" within a single unordered list item, preceeded by a blank line and just enough spaces.
        The blank line is required as LRDs cannot interrupt a paragraph.
    """

    # Arrange
    source_markdown = """-
  [fred]: /url
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::  ]",
        "[BLANK(1,2):]",
        "[link-ref-def(2,3):True::fred:: :/url:::::]",
        "[end-ulist:::True]",
        "[para(3,1):]",
        "[link(3,1):shortcut:/url:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li></li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_extra_within_ulist_leading_para() -> None:
    """
    Test case: "LRD" within a single unordered list item, preceeded by a blank line and more than enough spaces.
        The blank line is required as LRDs cannot interrupt a paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_extra_within_ulist
    """

    # Arrange
    source_markdown = """- abc

    [fred]: /url
[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n  ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:  :fred:: :/url:::::]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_split_two_lines_minus_all_spaces() -> (
    None
):
    """
    Test case extra 01:  LRD within list item
    """

    # Arrange
    source_markdown = """- [foo]:
/url"""
    expected_tokens = [
        "[ulist(1,1):-::2::]",
        "[para(1,3):\n]",
        "[text(1,3):[foo]:\n/url::\n]",
        "[end-para:::True]",
        "[end-ulist:::True]",
    ]
    expected_gfm = """<ul>
<li>[foo]:
/url</li>
</ul>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_split_two_lines_x() -> (
    None
):
    """
    Test case extra 01a:  variation on 1 with proper indent
    """

    # Arrange
    source_markdown = """- [foo]:
  /url"""
    expected_tokens = [
        "[ulist(1,1):-::2::  ]",
        "[link-ref-def(1,3):True::foo::\n:/url:::::]",
        "[end-ulist:::True]",
    ]
    expected_gfm = """<ul>
<li></li>
</ul>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_ulist_leading_para_split_two_lines_minus_some_spaces() -> (
    None
):
    """
    Test case extra 01b:  variation on 1 with almost proper indent

    NOTE: Due to https://talk.commonmark.org/t/block-quotes-laziness-and-link-reference-definitions/3751
          the GFM output has been adjusted to compensate for PyMarkdown using a token and not parsing
          the text afterwards.

    """

    # Arrange
    source_markdown = """- [foo]:
 /url"""
    expected_tokens = [
        "[ulist(1,1):-::2::]",
        "[para(1,3):\n ]",
        "[text(1,3):[foo]:\n/url::\n]",
        "[end-para:::True]",
        "[end-ulist:::True]",
    ]
    expected_gfm = """<ul>
<li>[foo]:
/url</li>
</ul>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_olist() -> None:
    """
    Test case: "LRD" within a single ordered list item, preceeded by just enough spaces.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_within_olist
    """

    # Arrange
    source_markdown = """1. abc

   [fred]: /url
[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n   ]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,4):True::fred:: :/url:::::]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_extra_within_olist() -> None:
    """
    Test case: "LRD" within a single ordered list item, preceeded by extra spaces.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_extra_within_olist
    """

    # Arrange
    source_markdown = """1. abc

    [fred]: /url
[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n   ]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True: :fred:: :/url:::::]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_block_quote() -> None:
    """
    Test case: LRD within a single block quote, preceeded by just enough spaces.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_within_block_quote
    """

    # Arrange
    source_markdown = """> [fred]: /url
> [fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,3):True::fred:: :/url:::::]",
        "[para(2,3):]",
        "[link(2,3):shortcut:/url:::::fred:False::::]",
        "[text(2,4):fred:]",
        "[end-link::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p><a href="/url">fred</a></p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_extra_within_block_quote() -> None:
    """
    Test case: LRD within a single block quote, preceeded by extra spaces.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_extra_within_block_quote
    """

    # Arrange
    source_markdown = """> abc
>  [fred]: /url
>  [fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n> ]",
        "[para(1,3):\n \n ]",
        "[text(1,3):abc\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
[fred]: /url
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_after_block_quote() -> None:
    """
    Test case: Table after text within single block quote, without any block quote start character.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_after_block_quote
    """

    # Arrange
    source_markdown = """> abc
> def
  [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n\n]",
        "[para(1,3):\n\n  \n]",
        "[text(1,3):abc\ndef\n[fred]: /url\n[fred]::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def
[fred]: /url
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_after_blank_line_and_block_quote() -> None:
    """
    Test case: Table after text within single block quote, without any block quote start character.

    Related: test_whitespaces_tables_extension_with_spaces_after_block_quote
    """

    # Arrange
    source_markdown = """> abc
> def

  [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        "[para(1,3):\n]",
        "[text(1,3):abc\ndef::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[link-ref-def(4,3):True:  :fred:: :/url:::::]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def</p>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_some_spaces_after_double_block_quotes() -> None:
    """
    Test case: "LRD" after text within double block quote, without any block quote start character.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: xxxxxxxxxxtexxxxst_whitespaces_tables_extension_with_spaces_after_block_quote
    """

    # Arrange
    source_markdown = """> > abc
> > def
  [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::]",
        "[block-quote(1,3)::> > \n> > \n\n]",
        "[para(1,5):\n\n  \n]",
        "[text(1,5):abc\ndef\n[fred]: /url\n[fred]::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<blockquote>
<p>abc
def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_extra_spaces_after_double_block_quotes() -> None:
    """
    Test case: "LRD" after text within double block quote, without any block quote start character.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_extra_spaces_after_double_block_quotes
    """

    # Arrange
    source_markdown = """> > abc
> > def
    [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::]",
        "[block-quote(1,3)::> > \n> > \n\n]",
        "[para(1,5):\n\n    \n]",
        "[text(1,5):abc\ndef\n[fred]: /url\n[fred]::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<blockquote>
<p>abc
def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


def test_whitespaces_lrd_with_spaces_within_single_block_quote_after_double_block_quote() -> (
    None
):
    """
    Test case: "LRD" after text within single block quote, occurring after a double block quote.
        As LRDs cannot interrupt a paragraph, this should be treated as an extension of the paragraph.

    Related: test_whitespaces_tables_extension_with_spaces_within_single_block_quote_after_double_block_quote
    """

    # Arrange
    source_markdown = """> abc
> > def
>   [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n> \n]",
        "[para(2,5):\n  \n]",
        "[text(2,5):def\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_and_single_space() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces.
    """

    # Arrange
    source_markdown = """1. abc
 \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:: \n]",
        "[para(1,4):\n\t]",
        "[text(1,4):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
[fred]: /url</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_and_spaces() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
  \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::  \n]",
        "[para(1,4):\n\t]",
        "[text(1,4):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
[fred]: /url</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_double_list_x() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
   1. def
\t  [fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[olist(2,4):.:1:6:   :\t  \n]",
        "[para(2,7):\n]",
        "[text(2,7):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
<ol>
<li>def
[fred]: /url</li>
</ol>
</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_double_list_no_spaces() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
   1. def
\t[fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[olist(2,4):.:1:6:   :\n]",
        "[para(2,7):\n\t]",
        "[text(2,7):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
<ol>
<li>def
[fred]: /url</li>
</ol>
</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_double_list_tab_after_indent() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
   1. def
   \t[fred]:\t/url\t

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[olist(2,4):.:1:6:   :\n]",
        "[para(2,7):\n   \t:\t]",
        "[text(2,7):def\n[fred]:\t/url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
<ol>
<li>def
[fred]:\t/url</li>
</ol>
</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_double_list_one_space() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
   1. def
\t [fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[olist(2,4):.:1:6:   :\n]",
        "[para(2,7):\n\t ]",
        "[text(2,7):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
<ol>
<li>def
[fred]: /url</li>
</ol>
</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_double_list_only_spaces() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
   1. def
    [fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3:]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[olist(2,4):.:1:6:   :\n]",
        "[para(2,7):\n    ]",
        "[text(2,7):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
<ol>
<li>def
[fred]: /url</li>
</ol>
</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_formfeeds_before_within_list_x() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
 \u000C \u000C[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n]",
        "[para(1,3):\n ]",
        "[text(1,3):abc\n\u000C \u000C[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
\u000C \u000C[fred]: /url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_formfeeds_before_within_list_a() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc

 \u000C \u000C[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[end-ulist:::True]",
        "[para(3,2): \u000C \u000C]",
        "[text(3,2):[fred]: /url:]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc</li>
</ul>
<p>[fred]: /url</p>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_double_block_quotes_with_zero_and_zero_spaces_at_start() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
[fred]: /url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n\n]",
        "[para(2,5):\n]",
        "[text(2,5):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url</p>
</blockquote>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_double_block_quotes_with_zero_and_one_space_at_start() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
 [fred]: /url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n\n]",
        "[para(2,5):\n ]",
        "[text(2,5):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url</p>
</blockquote>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_double_block_quotes_with_zero_and_two_spaces_at_start() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
  [fred]: /url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n\n]",
        "[para(2,5):\n  ]",
        "[text(2,5):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url</p>
</blockquote>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_double_block_quotes_with_zero_and_three_spaces_at_start() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
   [fred]: /url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n\n]",
        "[para(2,5):\n   ]",
        "[text(2,5):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url</p>
</blockquote>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_x1() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> def
\t[fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n\n]",
        "[para(1,3):\n\n\t\n]",
        "[text(1,3):abc\ndef\n[fred]: /url\n[fred]::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def
[fred]: /url
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_x2() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
> def
  \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n\n]",
        "[para(1,3):\n\n  \t]",
        "[text(1,3):abc\ndef\n[fred]: /url::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def
[fred]: /url</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_x3() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> def

\t[fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        "[para(1,3):\n]",
        "[text(1,3):abc\ndef::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[icode-block(4,5):\t:]",
        "[text(4,5):[fred]: /url:]",
        "[end-icode-block:::False]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def</p>
</blockquote>
<pre><code>[fred]: /url
</code></pre>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_repeat() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> def
>\t[fred]: /url1
>\t[barney]: /url2

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n>\n>\n]",
        "[para(1,3):\n\n\t\n\t]",
        "[text(1,3):abc\ndef\n[fred]: /url1\n[barney]: /url2::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(5,1):]",
        "[para(6,1):\n]",
        "[text(6,1):[fred]\n[barney]::\n]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def
[fred]: /url1
[barney]: /url2</p>
</blockquote>
<p>[fred]
[barney]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]: /url1
>\t[barney]: /url2
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>\n]",
        "[link-ref-def(1,5):True:\t:fred:: :/url1:::::]",
        "[link-ref-def(2,5):True:\t:barney:: :/url2:::::]",
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]: /url1
> \t[barney]: /url2
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        "[link-ref-def(1,5):True:\t:fred:: :/url1:::::]",
        "[link-ref-def(2,5):True:\t:barney:: :/url2:::::]",
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_many_tabs() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t/url1\t
>\t[barney]:\t/url2\t
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>\n]",
        "[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:::]",
        "[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:::]",
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=True)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_block_quote_just_enough() -> None:
    """
    Test case:  LRD preceeded by spaces.
    """

    # Arrange
    source_markdown = """> abc
> def
   [fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n\n]",
        "[para(1,3):\n\n   \n]",
        "[text(1,3):abc\ndef\n[fred]: /url\n[fred]::\n\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
def
[fred]: /url
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_1a() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]: /url1
>\t[barney]: /url2

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>]",
        "[link-ref-def(1,5):True:\t:fred:: :/url1:::::]",
        "[link-ref-def(2,5):True:\t:barney:: :/url2:::::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=False)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_1b() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]: /url1 "title"
>\t[barney]: /url2 "other title"
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>]",
        '[link-ref-def(1,5):True:\t:fred:: :/url1:: :title:"title":]',
        '[link-ref-def(2,5):True:\t:barney:: :/url2:: :other title:"other title":]',
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:title::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_1c() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]: /url1 "title"
>\t[barney]: /url2 "other title"

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>\n]",
        '[link-ref-def(1,5):True:\t:fred:: :/url1:: :title:"title":]',
        '[link-ref-def(2,5):True:\t:barney:: :/url2:: :other title:"other title":]',
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:title::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_2a() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t/url1\t
>\t[barney]:\t/url2\t

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>]",
        "[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:::]",
        "[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_2b() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t/url1\t"title"\t
>\t[barney]:\t/url2\t"other title"\t
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>]",
        '[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:title:"title":\t]',
        '[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:other title:"other title":\t]',
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:title::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_2c() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t/url1\t"title"\t
>\t[barney]:\t/url2\t"other title"\t

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>\n]",
        '[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:title:"title":\t]',
        '[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:other title:"other title":\t]',
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:title::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_repeat_3() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t/url1\t
>\t[barney]:\t/url2\t
>\t[fred]
>\t[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>\n>\n>]",
        "[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:::]",
        "[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:::]",
        "[para(3,5):\t\n\t]",
        "[link(3,5):shortcut:/url1:::::fred:False::::]",
        "[text(3,6):fred:]",
        "[end-link::]",
        "[text(3,11):\n::\n]",
        "[link(4,3):shortcut:/url2:::::barney:False::::]",
        "[text(4,4):barney:]",
        "[end-link::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_block_quotes_bare_repeat_3() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> [fred]: /url1
> [barney]: /url2
> [fred]
> [barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n> \n> ]",
        "[link-ref-def(1,3):True::fred:: :/url1:::::]",
        "[link-ref-def(2,3):True::barney:: :/url2:::::]",
        "[para(3,3):\n]",
        "[link(3,3):shortcut:/url1:::::fred:False::::]",
        "[text(3,4):fred:]",
        "[end-link::]",
        "[text(3,9):\n::\n]",
        "[link(4,3):shortcut:/url2:::::barney:False::::]",
        "[text(4,4):barney:]",
        "[end-link::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_1a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]: /url1
> \t[barney]: /url2

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,5):True:\t:fred:: :/url1:::::]",
        "[link-ref-def(2,5):True:\t:barney:: :/url2:::::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_1b() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]: /url1 "title"
> \t[barney]: /url2 "other title"
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        '[link-ref-def(1,5):True:\t:fred:: :/url1:: :title:"title":]',
        '[link-ref-def(2,5):True:\t:barney:: :/url2:: :other title:"other title":]',
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:title::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_1c() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]: /url1 "title"
> \t[barney]: /url2 "other title"

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        '[link-ref-def(1,5):True:\t:fred:: :/url1:: :title:"title":]',
        '[link-ref-def(2,5):True:\t:barney:: :/url2:: :other title:"other title":]',
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:title::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:other title::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other title">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_2x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t
> \t[barney]:\t/url2\t
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        "[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:::]",
        "[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:::]",
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_2a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t
> \t[barney]:\t/url2\t

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:::]",
        "[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=True)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_block_quotes_bare_with_space_repeat_2a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> [fred]: /url1
> [barney]: /url2

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,3):True::fred:: :/url1:::::]",
        "[link-ref-def(2,3):True::barney:: :/url2:::::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1">fred</a>
<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_2b() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"title"\t
> \t[barney]:\t/url2\t"other\ttitle"\t
[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        '[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:title:"title":\t]',
        '[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:other\ttitle:"other\ttitle":\t]',
        "[end-block-quote:::True]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:title::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:other\ttitle::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other\ttitle">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_2c() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"title"\t
> \t[barney]:\t/url2\t"other\ttitle"\t

[fred]
[barney]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n]",
        '[link-ref-def(1,5):True:\t:fred::\t:/url1::\t:title:"title":\t]',
        '[link-ref-def(2,5):True:\t:barney::\t:/url2::\t:other\ttitle:"other\ttitle":\t]',
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[para(4,1):\n]",
        "[link(4,1):shortcut:/url1:title::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[text(4,7):\n::\n]",
        "[link(5,1):shortcut:/url2:other\ttitle::::barney:False::::]",
        "[text(5,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url1" title="title">fred</a>
<a href="/url2" title="other\ttitle">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_3x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"abc"def
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t\n]",
        '[text(1,5):[fred]:\t/url1\t\a"\a&quot;\aabc\a"\a&quot;\adef\n[fred]::\n]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:	/url1	&quot;abc&quot;def
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_3a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"abc"def

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t]",
        '[text(1,5):[fred]:\t/url1\t\a"\a&quot;\aabc\a"\a&quot;\adef:]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(2,1):]",
        "[para(3,1):]",
        "[text(3,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:	/url1	&quot;abc&quot;def</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_4x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"abc
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t\n]",
        '[text(1,5):[fred]:\t/url1\t\a"\a&quot;\aabc\n[fred]::\n]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:\t/url1\t&quot;abc
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_4a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t/url1\t"abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t]",
        '[text(1,5):[fred]:\t/url1\t\a"\a&quot;\aabc:]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(2,1):]",
        "[para(3,1):]",
        "[text(3,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:\t/url1\t&quot;abc</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_5x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t</url1
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t\n]",
        "[text(1,5):[fred]:\t\a<\a&lt;\a/url1\n[fred]::\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:\t&lt;/url1
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_5a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t</url1

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t]",
        "[text(1,5):[fred]:\t\a<\a&lt;\a/url1:]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(2,1):]",
        "[para(3,1):]",
        "[text(3,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:\t&lt;/url1</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_6() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t\n]",
        "[text(1,5):[fred]:\n[fred]::\t\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:	
[fred]</p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_with_space_repeat_6a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,5):\t:\t]",
        "[text(1,5):[fred]::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(2,1):]",
        "[para(3,1):]",
        "[text(3,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>[fred]:</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1xx() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t
> \t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1xa() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:
> \t/url

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,5):True:\t:fred::\n\t:/url:::::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1xb() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> [fred]:\t
> /url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,3):True::fred::\t\n:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1ax() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t
> \t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n> ]",
        "[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=False)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1aa() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t
> \t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> ]",
        "[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1b() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t
>\t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>]",
        "[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1c() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """>\t[fred]:\t
>\t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::>\n>]",
        "[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[link(4,1):shortcut:/url:::::fred:False::::]",
        "[text(4,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1d() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>\t[fred]:\t
>\t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n>\n]",
        "[para(1,3):\n\t\n\t:\t]",
        "[text(1,3):abc\n[fred]:\n/url::\n\t\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
[fred]:	
/url</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1ea() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
>
>\t[fred]:\t
>\t/url\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n>]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n>]",
        "[para(2,5):]",
        "[text(2,5):def:]",
        "[end-para:::True]",
        "[BLANK(3,2):]",
        "[end-block-quote:::True]",
        "[link-ref-def(4,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[end-block-quote:::False]",
        "[BLANK(6,1):]",
        "[para(7,1):]",
        "[link(7,1):shortcut:/url:::::fred:False::::]",
        "[text(7,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def</p>
</blockquote>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_two_lines_1f() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> \t[fred]:\t
> \t/url\t
> abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[para(5,3):]",
        "[text(5,3):abc:]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(6,1):]",
        "[para(7,1):]",
        "[link(7,1):shortcut:/url:::::fred:False::::]",
        "[text(7,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>abc</p>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_more_lines_2x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> \t[fred]:\t
> \t/url\t
> \t"title"\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n> \n> \n]",
        '[link-ref-def(1,5):True:\t:fred::\t\n\t:/url::\t\n\t:title:"title":\t]',
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
</blockquote>
<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_more_lines_2a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> \t[fred]:\t
> \t/url\t
> \t"title"\t
> abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        '[link-ref-def(3,5):True:\t:fred::\t\n\t:/url::\t\n\t:title:"title":\t]',
        "[para(6,3):]",
        "[text(6,3):abc:]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(7,1):]",
        "[para(8,1):]",
        "[link(8,1):shortcut:/url:title::::fred:False::::]",
        "[text(8,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>abc</p>
</blockquote>
<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_more_lines_3x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> \t[fred]:\t
> \t/url\t
> \t"times\t
> \tnew\troman\t
> abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n> \n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[para(5,5):\t\n\t\n]",
        '[text(5,5):\a"\a&quot;\atimes\nnew\troman\nabc::\t\n\t\n]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(8,1):]",
        "[para(9,1):]",
        "[link(9,1):shortcut:/url:::::fred:False::::]",
        "[text(9,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>&quot;times	
new	roman	
abc</p>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=False)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_block_quotes_bare_over_more_lines_3x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> [fred]:
> /url
> "times
> new roman
> abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n> \n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[link-ref-def(3,3):True::fred::\n:/url:::::]",
        "[para(5,3):\n\n]",
        '[text(5,3):\a"\a&quot;\atimes\nnew roman\nabc::\n\n]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(8,1):]",
        "[para(9,1):]",
        "[link(9,1):shortcut:/url:::::fred:False::::]",
        "[text(9,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>&quot;times
new roman
abc</p>
</blockquote>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=False)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_within_block_quotes_bare_over_more_lines_3xa() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> [fred]:
> /url
> "times
> new roman
> abc
>
> [fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n> \n> \n>\n> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[link-ref-def(3,3):True::fred::\n:/url:::::]",
        "[para(5,3):\n\n]",
        '[text(5,3):\a"\a&quot;\atimes\nnew roman\nabc::\n\n]',
        "[end-para:::True]",
        "[BLANK(8,2):]",
        "[para(9,3):]",
        "[link(9,3):shortcut:/url:::::fred:False::::]",
        "[text(9,4):fred:]",
        "[end-link::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>&quot;times
new roman
abc</p>
<p><a href="/url">fred</a></p>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens, show_debug=False)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_more_lines_3a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> \t[fred]:\t
> \t</url\t
> abc

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n> \n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[para(3,5):\t\n\t\n]",
        "[text(3,5):[fred]:\n\a<\a&lt;\a/url\nabc::\t\n\t\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(6,1):]",
        "[para(7,1):]",
        "[text(7,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>[fred]:	
&lt;/url	
abc</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_block_quotes_bare_over_more_lines_3b() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
>
> \t[fred]:\t

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        "[para(3,5):\t:\t]",
        "[text(3,5):[fred]::]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<p>[fred]:</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_double_block_quotes() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
\t[fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n\n]",
        "[para(2,5):\n\t\n]",
        "[text(2,5):def\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_double_block_quotes_with_single() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
>\t[fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n>\n]",
        "[para(2,5):\n\t\n]",
        "[text(2,5):def\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_double_block_quotes_with_single_and_space() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """> abc
> > def
> \t[fred]: /url
[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> ]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[block-quote(2,1)::> > \n> \n]",
        "[para(2,5):\n\t\n]",
        "[text(2,5):def\n[fred]: /url\n[fred]::\n\n]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[end-block-quote:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
<blockquote>
<p>def
[fred]: /url
[fred]</p>
</blockquote>
</blockquote>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_label() -> None:
    """
    Test case:  LRD link label preceeded by spaces.
    """

    # Arrange
    source_markdown = """[  fred]: /url
[ fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:  fred: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url::::: fred:False::::]",
        "[text(2,2): fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url"> fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_label() -> None:
    """
    Test case:  LRD link label preceeded by tabs.
    """

    # Arrange
    source_markdown = """[\t\tfred]: /url
[ fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:\t\tfred: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url::::: fred:False::::]",
        "[text(2,2): fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url"> fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_before_label() -> None:
    """
    Test case:  LRD link label preceeded by form feeds.
    """

    # Arrange
    source_markdown = """[\u000cfred]: /url
[ fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:\u000cfred: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url::::: fred:False::::]",
        "[text(2,2): fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url"> fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_after_label() -> None:
    """
    Test case:  LRD link label followed by spaces.
    """

    # Arrange
    source_markdown = """[fred  ]: /url
[fred ]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:fred  : :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred :False::::]",
        "[text(2,2):fred :]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred </a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_after_label() -> None:
    """
    Test case:  LRD link label followwed by tabs.
    """

    # Arrange
    source_markdown = """[fred\t\t]: /url
[fred ]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:fred\t\t: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred :False::::]",
        "[text(2,2):fred :]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred </a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_after_label() -> None:
    """
    Test case:  LRD link label followed by form feeds.
    """

    # Arrange
    source_markdown = """[fred\u000c]: /url
[fred ]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:fred\u000c: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred :False::::]",
        "[text(2,2):fred :]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred </a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_in_label() -> None:
    """
    Test case:  LRD link label with spaces inside.
    """

    # Arrange
    source_markdown = """[fred  boy]: /url
[fred boy]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred boy:fred  boy: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred boy:False::::]",
        "[text(2,2):fred boy:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred boy</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_in_label() -> None:
    """
    Test case:  LRD link label followwed by tabs.
    """

    # Arrange
    source_markdown = """[fred\t\tboy]: /url
[fred boy]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred boy:fred\t\tboy: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred boy:False::::]",
        "[text(2,2):fred boy:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred boy</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_in_label() -> None:
    """
    Test case:  LRD link label followed by form feeds.
    """

    # Arrange
    source_markdown = """[fred\u000c\u000cboy]: /url
[fred boy]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred boy:fred\u000c\u000cboy: :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred boy:False::::]",
        "[text(2,2):fred boy:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred boy</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_destination() -> None:
    """
    Test case:  LRD destination preceeded by spaces.
    """

    # Arrange
    source_markdown = """[fred]:  /url
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred::  :/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_destination() -> None:
    """
    Test case:  LRD destination preceeded by tabs.
    """

    # Arrange
    source_markdown = """[fred]:\t/url
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred::\t:/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_before_destination() -> None:
    """
    Test case:  LRD destination preceeded by form feeds.
    """

    # Arrange
    source_markdown = """[fred]:\u000c/url
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred::\u000c:/url:::::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_after_destination() -> None:
    """
    Test case:  LRD destination followed by spaces.
    """

    # Arrange
    source_markdown = """[fred]: /url\a\a
[fred]""".replace(
        "\a", " "
    )
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url::  :::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_after_destination() -> None:
    """
    Test case:  LRD destination followed by tabs.
    """

    # Arrange
    source_markdown = """[fred]: /url\t\t
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url::\t\t:::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_after_destination() -> None:
    """
    Test case:  LRD destination followed by form feeds.
    """

    # Arrange
    source_markdown = """[fred]: /url\u000c\u000c
[fred]"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url::\u000c\u000c:::]",
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_title() -> None:
    """
    Test case:  LRD title preceeded by spaces.
    """

    # Arrange
    source_markdown = """[fred]: /url "title"
[fred]"""
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url:: :title:"title":]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_title() -> None:
    """
    Test case:  LRD title preceeded by tabs.
    """

    # Arrange
    source_markdown = """[fred]: /url\t"title"
[fred]"""
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url::\t:title:"title":]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_before_title() -> None:
    """
    Test case:  LRD title preceeded by form feeds.
    """

    # Arrange
    source_markdown = """[fred]: /url\u000c"title"
[fred]"""
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url::\u000c:title:"title":]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_after_title() -> None:
    """
    Test case:  LRD title followed by spaces.
    """

    # Arrange
    source_markdown = """[fred]: /url "title"\a\a\a
[fred]""".replace(
        "\a", " "
    )
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url:: :title:"title":   ]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_after_title() -> None:
    """
    Test case:  LRD title followed by tabs.
    """

    # Arrange
    source_markdown = """[fred]: /url "title"\t\t
[fred]"""
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url:: :title:"title":\t\t]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_form_feeds_after_title() -> None:
    """
    Test case:  LRD title followed by form feeds.
    """

    # Arrange
    source_markdown = """[fred]: /url "title"\u000c\u000c
[fred]"""
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: :/url:: :title:"title":\u000c\u000c]',
        "[para(2,1):]",
        "[link(2,1):shortcut:/url:title::::fred:False::::]",
        "[text(2,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_x() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
\t[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n]",
        "[para(1,3):\n\t]",
        "[text(1,3):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]: /url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_multiple_tabs_xx() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
\t[fred]:\t/url\t"bob\t"

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n]",
        "[para(1,3):\n\t]",
        '[text(1,3):abc\n[fred]:\t/url\t\a"\a&quot;\abob\t\a"\a&quot;\a::\n]',
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]:\t/url\t&quot;bob\t&quot;</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_multiple_tabs_xx() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc
\t[fred]:\t/url\t"bob\t"

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n]",
        "[para(1,4):\n\t]",
        '[text(1,4):abc\n[fred]:\t/url\t\a"\a&quot;\abob\t\a"\a&quot;\a::\n]',
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
[fred]:\t/url\t&quot;bob\t&quot;</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_multiple_tabs_xax() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
\t[fred]:\t/url\t"bob\t"

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n\n]",
        "[para(1,3):\n\t]",
        '[text(1,3):abc\n[fred]:\t/url\t\a"\a&quot;\abob\t\a"\a&quot;\a::\n]',
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(3,1):]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc
[fred]:\t/url\t&quot;bob\t&quot;</p>
</blockquote>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_multiple_tabs_xaa() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc

\t[fred]:\t/url\t"bob\t"

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[end-block-quote:::True]",
        "[BLANK(2,1):]",
        "[icode-block(3,5):\t:]",
        '[text(3,5):[fred]:\t/url\t\a"\a&quot;\abob\t\a"\a&quot;\a:]',
        "[end-icode-block:::False]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
</blockquote>
<pre><code>[fred]:\t/url\t&quot;bob\t&quot;
</code></pre>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_multiple_tabs_xab() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """> abc
>
>\t[fred]:\t/url\t"bob\t"

[fred]"""
    expected_tokens = [
        "[block-quote(1,1)::> \n>\n>\n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,2):]",
        '[link-ref-def(3,5):True:\t:fred::\t:/url::\t:bob\t:"bob\t":]',
        "[end-block-quote:::True]",
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:bob\t::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<blockquote>
<p>abc</p>
</blockquote>
<p><a href="/url" title="bob\t">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_multiple_tabs_a() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc

\t[fred]:\t/url\t"bob"\t

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n\n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        '[link-ref-def(3,5):True:\t:fred::\t:/url::\t:bob:"bob":\t]',
        "[BLANK(4,1):]",
        "[end-ulist:::True]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:bob::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url" title="bob">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_multiple_tabs_a() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """1. abc

\t[fred]:\t/url\t"bob"\t

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n\n]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        '[link-ref-def(3,5):True:\t:fred::\t:/url::\t:bob:"bob":\t]',
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:bob::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url" title="bob">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_and_single_space() -> (
    None
):
    """
    Test case:  LRD preceeded by spaces.
    """

    # Arrange
    source_markdown = """- abc
 \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2:: \n]",
        "[para(1,3):\n\t]",
        "[text(1,3):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]: /url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_and_spaces_x() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
  \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::  \n]",
        "[para(1,3):\n\t]",
        "[text(1,3):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]: /url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_and_spaces_a() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
  \t[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::  \n]",
        "[para(1,3):\n\t]",
        "[text(1,3):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-ulist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]: /url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_double_list() -> None:
    """
    Test case:  LRD preceeded by spaces and tabs.
    """

    # Arrange
    source_markdown = """- abc
  - def
\t[fred]: /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2:]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[ulist(2,3):-::4:  :\t\n]",
        "[para(2,5):\n]",
        "[text(2,5):def\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-ulist:::True]",
        "[end-ulist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
<ul>
<li>def
[fred]: /url</li>
</ul>
</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_x() -> None:
    """
    Test case:  LRD preceeded by spaces.
    """

    # Arrange
    source_markdown = """1. abc
\t[fred]: /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n]",
        "[para(1,4):\n\t]",
        "[text(1,4):abc\n[fred]: /url::\n]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[end-olist:::True]",
        "[para(4,1):]",
        "[text(4,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
[fred]: /url</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_single_over_two_lines_x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """- abc

\t[fred]:\t
  /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n\n  \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n:/url:::::]",
        "[BLANK(5,1):]",
        "[end-ulist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_single_over_two_lines_x() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """1. abc

\t[fred]:\t
   /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n\n   \n]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n:/url:::::]",
        "[BLANK(5,1):]",
        "[end-olist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_single_over_two_lines_a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """- abc

 \t[fred]:
  /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n \n  \n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\n:/url:::::]",
        "[BLANK(5,1):]",
        "[end-ulist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_single_over_two_lines_a() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """1. abc

 \t[fred]:
   /url

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n \n   \n]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\n:/url:::::]",
        "[BLANK(5,1):]",
        "[end-olist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_list_single_over_two_lines_b() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """- abc
  - def

  \t[fred]:
    /url

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2:]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[ulist(2,3):-::4:  :\n  \n    \n]",
        "[para(2,5):]",
        "[text(2,5):def:]",
        "[end-para:::True]",
        "[BLANK(3,1):]",
        "[link-ref-def(4,5):True:\t:fred::\n:/url:::::]",
        "[BLANK(6,1):]",
        "[end-ulist:::True]",
        "[end-ulist:::True]",
        "[para(7,1):]",
        "[link(7,1):shortcut:/url:::::fred:False::::]",
        "[text(7,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
<ul>
<li>
<p>def</p>
</li>
</ul>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_bare_over_two_lines_1ea() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """- abc
\t[fred]:\t
\t/url\t

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n\n]",
        "[para(1,3):\n\t\n\t:\t]",
        "[text(1,3):abc\n[fred]:\n/url::\n\t\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-ulist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>abc
[fred]:	
/url</li>
</ul>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_unordered_list_over_two_lines_1ea() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """- abc

\t[fred]:\t
\t/url\t

[fred]"""
    expected_tokens = [
        "[ulist(1,1):-::2::\n\n\n]",
        "[para(1,3):]",
        "[text(1,3):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[BLANK(5,1):]",
        "[end-ulist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ul>
<li>
<p>abc</p>
</li>
</ul>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_bare_over_two_lines_1ea() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """1. abc
\t[fred]:\t
\t/url\t

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n\n]",
        "[para(1,4):\n\t\n\t:\t]",
        "[text(1,4):abc\n[fred]:\n/url::\n\t\n]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
        "[end-olist:::True]",
        "[para(5,1):]",
        "[text(5,1):[fred]:]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>abc
[fred]:\t
/url</li>
</ol>
<p>[fred]</p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_tabs_before_within_ordered_list_over_two_lines_1ea() -> (
    None
):
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """1. abc

\t[fred]:\t
\t/url\t

[fred]"""
    expected_tokens = [
        "[olist(1,1):.:1:3::\n\n\n]",
        "[para(1,4):]",
        "[text(1,4):abc:]",
        "[end-para:::True]",
        "[BLANK(2,1):]",
        "[link-ref-def(3,5):True:\t:fred::\t\n\t:/url::\t:::]",
        "[BLANK(5,1):]",
        "[end-olist:::True]",
        "[para(6,1):]",
        "[link(6,1):shortcut:/url:::::fred:False::::]",
        "[text(6,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<ol>
<li>
<p>abc</p>
</li>
</ol>
<p><a href="/url">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_same() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_tables_extension_198_enabled_with_leading_same
    """

    # Arrange
    source_markdown = """ [fred]:
 /url
 "title"

[fred]"""
    expected_tokens = [
        '[link-ref-def(1,2):True: :fred::\n :/url::\n :title:"title":]',
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_before_different() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_tables_extension_198_enabled_with_leading_different
    """

    # Arrange
    source_markdown = """ [fred]:
/url
   "title"

[fred]"""
    expected_tokens = [
        '[link-ref-def(1,2):True: :fred::\n:/url::\n   :title:"title":]',
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_trailing_single() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_tables_extension_198_enabled_with_trailing_single
    """

    # Arrange
    source_markdown = """[fred]:\a
/url\a
"title"\a

[fred]""".replace(
        "\a", " "
    )
    expected_tokens = [
        '[link-ref-def(1,1):True::fred:: \n:/url:: \n:title:"title": ]',
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_trailing_double() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_tables_extension_198_enabled_with_trailing_double
    """

    # Arrange
    source_markdown = """[fred]:\a\a
/url\a\a
"title"\a\a

[fred]""".replace(
        "\a", " "
    )
    expected_tokens = [
        '[link-ref-def(1,1):True::fred::  \n:/url::  \n:title:"title":  ]',
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_trailing_triple() -> None:
    """
    Test case:  LRD preceeded by tabs.

    test_tables_extension_198_enabled_with_trailing_triple
    """

    # Arrange
    source_markdown = """[fred]:\a\a\a
/url\a\a\a
"title"\a\a\a

[fred]""".replace(
        "\a", " "
    )
    expected_tokens = [
        '[link-ref-def(1,1):True::fred::   \n:/url::   \n:title:"title":   ]',
        "[BLANK(4,1):]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url:title::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
    ]
    expected_gfm = """<p><a href="/url" title="title">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_followed_by_lrd() -> None:
    """
    Test case:  LRD preceeded by tabs.

    - test_tables_extension_201_enabled_fx
    """

    # Arrange
    source_markdown = """[fred]: /url1
[barney]: /url2
[fred]
[barney]
"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url1:::::]",
        "[link-ref-def(2,1):True::barney:: :/url2:::::]",
        "[para(3,1):\n]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[text(3,7):\n::\n]",
        "[link(4,1):shortcut:/url2:::::barney:False::::]",
        "[text(4,2):barney:]",
        "[end-link::]",
        "[end-para:::True]",
        "[BLANK(5,1):]",
    ]
    expected_gfm = """<p><a href="/url1">fred</a>\n<a href="/url2">barney</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_followed_by_table() -> None:
    """
    Test case:  LRD preceeded by tabs.
    """

    # Arrange
    source_markdown = """[fred]: /url1
| abc | def |
|-----|-----|
| ghi | jkl |
[fred]
"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url1:::::]",
        "[table(2,1)]",
        "[table-header(2,1):::True:|-----|-----|]",
        "[table-header-item(2,3): :]",
        "[text(2,3):abc:]",
        "[end-table-header-item: |::False]",
        "[table-header-item(2,9): :]",
        "[text(2,9):def:]",
        "[end-table-header-item: |::False]",
        "[end-table-header:::False]",
        "[table-body(4,1)]",
        "[table-row(4,1):::True:0]",
        "[table-row-item(4,3): :]",
        "[text(4,3):ghi:]",
        "[end-table-row-item: |::False]",
        "[table-row-item(4,9): :]",
        "[text(4,9):jkl:]",
        "[end-table-row-item: |::False]",
        "[end-table-row:::False]",
        "[end-table-body:::False]",
        "[end-table:::False]",
        "[para(5,1):]",
        "[link(5,1):shortcut:/url1:::::fred:False::::]",
        "[text(5,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
        "[BLANK(6,1):]",
    ]
    expected_gfm = """<table>
<thead>
<tr>
<th>abc</th>
<th>def</th>
</tr>
</thead>
<tbody>
<tr>
<td>ghi</td>
<td>jkl</td>
</tr>
</tbody>
</table>
<p><a href="/url1">fred</a></p>"""

    # Act & Assert
    act_and_assert(
        source_markdown,
        expected_gfm,
        expected_tokens,
        config_map=enable_tables_config_map,
    )


@pytest.mark.gfm
def test_whitespaces_lrd_with_spaces_followed_by_thematic() -> None:
    """
    Test case:  LRD preceeded by tabs.

    - test_tables_extension_201_enabled_bx
    """

    # Arrange
    source_markdown = """[fred]: /url1
---
[fred]
"""
    expected_tokens = [
        "[link-ref-def(1,1):True::fred:: :/url1:::::]",
        "[tbreak(2,1):-::---]",
        "[para(3,1):]",
        "[link(3,1):shortcut:/url1:::::fred:False::::]",
        "[text(3,2):fred:]",
        "[end-link::]",
        "[end-para:::True]",
        "[BLANK(4,1):]",
    ]
    expected_gfm = """<hr />
<p><a href="/url1">fred</a></p>"""

    # Act & Assert
    act_and_assert(source_markdown, expected_gfm, expected_tokens)
