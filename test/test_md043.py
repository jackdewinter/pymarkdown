...
@pytest.mark.rules
def test_md043_bad_configuration_headings_empty():
    """
    Test to make sure we get the expected behavior after scanning a good file from the
    test/resources/rules/md004 directory that has consistent asterisk usage on a single
    level list.
...
@pytest.mark.rules
def test_md043_bad_good_simple_headings_1_star_3_start_3():
    """
    Test to make sure we get the expected behavior after scanning a good file from the
    test/resources/rules/md004 directory that has consistent asterisk usage on a single
    level list.
    ...
    expected_return_code = 1
    expected_output = (
        "test/resources/rules/md043/good_simple_headings.md:3:1: "
        + "MD043: Required heading structure "
        + "[More than one wildcard not supported.] (required-headings,required-headers)"
    )
    expected_error = ""
