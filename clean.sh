...
parse_command_line() {
    ...
    if [[ $SOURCERY_ONLY_MODE -ne 0 ]] && [[ $MYPY_ONLY_MODE -ne 0 ]] ; then
        echo "Options '-m|--mypy-only' and '-s,--sourcery-only' conflict with each other.  Please choose one and try again."
        exit 1
    fi
}
...
if [[ $SOURCERY_ONLY_MODE -eq 0 ]] ; then
    execute_pre_commit
